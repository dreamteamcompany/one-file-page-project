...
CASE
  ...
  -- Бухгалтерия и ЗУП — отдельные базы, проверяем их ДО «1С».
  WHEN x ~ '(^|[^а-яёa-z])зуп|(^|[^а-яёa-z])бух|зарплат|ncalayer|документооборот' THEN '1С / Бухгалтерия / ЗУП'
  -- МИС работает на платформе 1С, пользователи так её и называют.
  WHEN x ~ '1с' THEN 'МИС'
  -- База знаний живёт в Битриксе.
  WHEN x ~ 'битрикс|bitrix|воронк|(^|[^а-яёa-z])лид|сделк|стади|баз\\w* знаний' THEN 'Битрикс / CRM'
  -- Voximplant — платформа телефонии.
  WHEN x ~ 'телефон|звонк|дозвон|ватс|whatsapp|вазап|этикетк|заспамлен|рассылк|(^|[^а-яёa-z])смс|sms|номер|voximplant|воксимплант|(^|[^a-z])sip([^a-z]|$)|речев\\w* аналитик' THEN 'Телефония и рассылки'
  -- Сеть, удалёнка и подключения. Сюда же «принтер по вай-фаю» и «документ
  -- не уходит на печать»: чинят это как проблему подключения, а не как
  -- поломку железа и не как вопрос по самой программе.
  WHEN x ~ 'vpn|впн|удаленк|удал[её]нн|удал[еёя]н\\w* (рабоч|стол|комп|доступ|подключ)|уд\\.стол|уд\\.? ?раб|рабоч\\w* стол|anydesk|анидеск|энидеск|интернет|wi-?fi|вай.?фа|сетев\\w* папк|серв[ае]р|терминал|хостинг|виртуалк' THEN 'Серверы и инфраструктура'
  -- Доступы к серверам по SSH и виртуальные машины.
  WHEN x ~ '(^|[^а-яёa-z])ssh([^а-яёa-z]|$)|виртуальн\\w* машин|(^|[^а-яёa-z])вм([^а-яёa-z]|$)' THEN 'Серверы и инфраструктура'
  WHEN x ~ 'не отправляется на печать|не уход[ия]т на печать|проблем.{0,4} с печать|не печатает|печать не работает|не работает печать' THEN 'Серверы и инфраструктура'
  ...
  -- Только про железо. «Печать», «распечатать», «компьютер» сами по себе
  -- к оборудованию не относятся: чаще это документы и шаблоны в МИС
  -- («не можем распечатать документы», «вошёл с другого компьютера»).
  WHEN x ~ 'принтер|сканер|картридж|монитор|ноутбук|оборудован|(^|[^а-яёa-z])касс[ауые]' THEN 'Оборудование'
  -- Аналитика и BI — отдельные внешние сервисы, не наши системы.
  WHEN x ~ 'аналитик|power ?bi|(^|[^а-яёa-z])bi([^а-яёa-z]|$)' THEN 'Другое'
  -- Система в тексте не названа (шаблонные заявки формы: «Предоставить доступ
  -- к системе», «Сообщить о проблеме»). По решению заказчика такие относим
...
CASE
  ...
  WHEN x ~ 'не работает|ошибк|не открывается|не проводятся|не подтягива|виснет|вис[ня]|непредвиденная|сбой|не приходят|не формир' THEN 'Ошибки и проблемы'
...
# о сбое. По словам из описания такие расходились по разным типам («доступ»,
# «войти», «настроить»), хотя человек сообщал о проблеме. Переносим их
# в «Ошибки и проблемы» во всех сервисах и группах.
ISSUE_OVERRIDE = """
CASE
  WHEN x ~ '^\\s*сообщить о проблеме' THEN 'Ошибки и проблемы'
  ELSE issue
END
"""
...
CASE
  ...
  -- 2-я линия ТП тоже не занимается МИС, но единой основной системы у неё
  -- нет (телефония и Битрикс примерно поровну) — нераспознанное уводим
  -- в «Другое», чтобы не завышать ни один из сервисов.
  WHEN line = '2-я линия ТП' AND service = 'МИС' THEN 'Другое'
  -- Отдел МИС занимается только МИС: что бы ни было написано в тексте
  -- заявки (1С, бухгалтерия, доступы), работа идёт по медицинской системе.
  WHEN line = 'Отдел МИС' THEN 'МИС'
  ELSE service
END
"""
